...
for origin in config.origins:
    for fx in config.fxs.keys():
        ...
        # load model
        fx_invert = model.FxInvert(n_param=n_param)
        fx_invert.to(device)

        # pretrain
        train.pretrain(pt_loader, fx_invert, config.n_epoch_pt, config.lr_pt, criterion.loss_pretrain, log_path_pt, state_path_pt, device)

        # evaluate
        eval.eval_pretrain(eval_loader, fx_invert, fx, eval_path_pt, device)

        # finetune
        train.finetune(ft_loader, fx_invert, config.n_epoch_ft, config.lr_ft, criterion.loss_finetune, log_path_ft, state_path_ft, device)

        # evaluate
        eval.eval_finetune(eval_loader, fx_invert, fx, eval_path_ft, device)
